...
#' Interpolation
#'
#' @description This function applies interpolation to fill in missing (or non-recorded) values.
#'
...
#' @param out_name optional. String with new column name. If you set it as \code{NULL}, the
#' function will overwrite the original data frame.
#' @param miss_table data frame with three columns: first and last date of interpolation
...
#' @param threshold numeric variable with the maximum number of dates in which to apply
#' the interpolation.
...
interpolate <- function(x,
  ...
  check_string(argument = col_name,
               target = colnames(x)[-1],
               arg_name = 'col_name')
  check_length(argument = col_name, max_allow = 1, arg_name = 'col_name')
  ...
  #* miss_table
  check_class(argument = miss_table, target = 'data.frame', arg_name = 'miss_table')
  check_class(argument = miss_table[ , 1], target = c('Date', 'POSIXct'),
              arg_name = 'miss_table[ , 1]')
  check_class(argument = miss_table[ , 2], target = c('Date', 'POSIXct'),
              arg_name = 'miss_table[ , 2]')
  check_class(argument = miss_table[ , 3], target = 'numeric',
  ...
  #* threshold
  check_class(argument = threshold, target = 'numeric', arg_name = 'threshold')
  check_length(argument = threshold, max_allow = 1, arg_name = 'threshold')

  #* method
  check_class(argument = method, target = 'character', arg_name = 'method')
  check_string(argument = method, target = 'linear', arg_name = 'method')
